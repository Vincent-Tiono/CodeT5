...
def front_is_clear(x):
    return x.front_is_clear

def left_is_clear(x):
    return x.left_is_clear

def right_is_clear(x):
    return x.right_is_clear

def marker_present(x):
    return x.marker_present

def no_marker_present(x):
    return x.no_marker_present
    ...
    def __init__(self, environment='karel'):
        if environment == 'karel':
            ...
            self.conditional_functions_dict = {
                self.t_FRONT_IS_CLEAR: front_is_clear, self.t_LEFT_IS_CLEAR: left_is_clear,
                self.t_RIGHT_IS_CLEAR: right_is_clear, self.t_MARKERS_PRESENT: marker_present,
                self.t_NO_MARKERS_PRESENT: no_marker_present,
            }
