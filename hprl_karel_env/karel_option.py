...
class Karel_world(object):
    ...
    def state_transition(self, a):
        ...
        if a_idx == 0:
            ...
            if self.env_task == 'topOff' or self.env_task == 'topOff_sparse':
...
class KarelEvaluation(Karel_world):
    def __init__(self, s=None, make_error=True, env_task="program", task_definition='program' ,reward_diff=False, final_reward_scale=True):
        super(KarelEvaluation, self).__init__(s=s, make_error=make_error, env_task=env_task, task_definition=task_definition, reward_diff=reward_diff, final_reward_scale=final_reward_scale)
        self.max_step = 100
        self.perception_count = 0
        self.max_perception = 2 * len(self.get_perception_list()) * self.max_step # true and false for each perception, the combination of all possible perception
        self.done = False
    ...
    def add_to_history(self, a_idx, agent_pos, made_error=False):
        self.elapse_step += 1
        self.s_h.append(self.s.copy())
        if self.elapse_step > self.max_step:
            self.done = True
            return

    def neighbor_is_clear(self, face):
        self.perception_count += 1
        if self.perception_count > self.max_perception:
            self.done = True
            raise RuntimeError("Maximum perception count reached.")
        neighbor_loc = self.get_neighbor(face)
        if neighbor_loc[0] >= self.h or neighbor_loc[0] < 0 \
                or neighbor_loc[1] >= self.w or neighbor_loc[1] < 0:
            return False
    ...
    def state_transition(self, a):
        super(KarelEvaluation, self).state_transition(a)
        if self.done:
            raise RuntimeError("Karel is done before program is finished.")
